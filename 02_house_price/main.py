...
def train_model(model_name, X_train, y_train, X_valid, y_valid, feature_list):
    ...
    if model_name == "xgboost":
        ...
        predictor = mlp.MLPModel(feature_list)
...
def main():
    ...
    for i in range(k):
        ...
        log.info(f"{config.metric} - {i+1}/{k}: {train_score=:.3f}, {valid_score=:.3f}, {spend_time=:.1f}s")
    ...
    log.info(f"{config.metric} - {k}_fold average: {train_score=:.3f}, {valid_score=:.3f}")
