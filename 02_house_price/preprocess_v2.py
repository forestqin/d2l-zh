...
def get_feature_df(train_input, test_input, create_feature=True):
    t = utils.Timer()
    ...
    log.info(f'Testing  Shape: {test_X.shape}, {test_Id.shape}')
    log.info(f"process data in {t.stop():.1f}s")

    if create_feature:
        train_X, test_X = create_features(train_X, train_y, test_X)
        log.info(f'Training Shape(create feature): {train_X.shape}, {train_y.shape}')
        log.info(f'Testing  Shape(create feature): {test_X.shape}, {test_Id.shape}')
        log.info(f"create feature in {t.stop():.1f}s")

    return train_X, train_y, test_X, test_Id, feature_list

def base_algorithm():
    train_df, train_label, test_df, test_Id, feature_list = get_feature_df(config.train_input, config.test_input, False)
    X = train_df.copy()
    y = train_label

    xgb = XGBRegressor()
    baseline_score = utils.score_dataset(X, y, xgb)
    print(f"Baseline score: {baseline_score:.5f} RMSLE")

    xgb = XGBRegressor()
    mi_scores = make_mi_scores(X, y)
    X = drop_uninformative(X, mi_scores)
    score = utils.score_dataset(X, y, xgb)
...
if __name__ == "__main__":
    ...
    xgb = XGBRegressor()
    score = utils.score_dataset(train_X, train_y, xgb)
